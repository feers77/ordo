"""Entorno de catálogo: el mismo tenant comercial completo."""
...
async def product_db_url() -> AsyncIterator[str]:
    db_name = f"ordo_product_test_{uuid.uuid4().hex[:8]}"
...
@pytest.fixture
async def shop(product_db_url: str) -> AsyncIterator[dict[str, Any]]:
    tenant = f"prd{uuid.uuid4().hex[:8]}"
    engine = create_async_engine(product_db_url)
